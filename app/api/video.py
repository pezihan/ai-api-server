...
class Text2Video(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            negative_prompt = negative_prompt.strip() if (negative_prompt and negative_prompt.strip()) else '人物动作卡顿不连贯, 肢体摆动僵硬怪异, 走路姿势畸形, 关节弯折角度反常, 帧间人物位置瞬移, 人物肢体虚影重影, 多手指, 少手指, 手指粘连扭曲, 手掌畸形, 多余肢体, 缺失肢体, 关节错位脱臼, 面部不对称, 五官扭曲变形, 眼睛错位/斗鸡眼/无眼, 鼻子歪斜, 嘴巴抽搐, 表情诡异僵硬, 皮肤反光过强像塑料, 皮肤纹理断层, 过度磨皮无细节, 服装穿模穿透身体, 漂浮衣物, 衣服褶皱不随动作变化, 衣料质感失真, 头发漂浮打结/帧间乱跳, 发饰脱落瞬移, 头身比例失调, 躯干过长/过短, 身体部件融合重叠, 人物边缘锯齿状, 人物与背景边界模糊, 画面卡顿跳帧, 帧间画面错位撕裂, 画面模糊发虚, 低分辨率马赛克, 动态模糊过度, 画面闪烁频闪, 色彩突变失真, 光影帧间不一致, 无阴影/阴影瞬移, 过曝/欠曝, 背景元素瞬移消失, 背景扭曲变形, 漂浮物体, 物体融合重叠, 现代元素乱入古风/写实场景, 画面出现无意义文字水印, AI伪影噪点, 颗粒感严重, 构图混乱无逻辑, 画面边缘黑边闪烁, 音画不同步, 背景音效刺耳杂乱, 镜头运动忽快忽慢, 镜头旋转眩晕感, 画面缩放比例失调'
